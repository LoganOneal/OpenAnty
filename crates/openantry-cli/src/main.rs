...
use openantry_core::{paths, OpenAntryService};
use openantry_proto::*;
use std::path::PathBuf;

#[derive(Parser, Debug)]
#[command(name = "openantry", version, about = "OpenAntry CLI — agent-first antidetect browser control")]
struct Cli {
    #[arg(long, global = true, env = "OPENANTRY_DATA_DIR")]
    data_dir: Option<PathBuf>,

    #[arg(long, global = true, env = "OPENANTRY_API_BASE")]
...
enum Commands {
    ...
    McpConfig {
        #[arg(long, default_value = "openantryd")]
        command: String,
    },
    /// Profile operations
    Profile {
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Init => {
            let (svc, recovery) = OpenAntryService::init(data_dir.clone())
                .map_err(|e| anyhow::anyhow!(e.to_string()))?;
            println!("Initialized OpenAntry at {}", svc.data_dir.display());
        ...
        Commands::Doctor { json } => {
            ...
            if json {
                ...
                println!("OpenAntry doctor — ok={}", report["ok"]);
...
fn open_svc(data_dir: PathBuf) -> Result<OpenAntryService> {
    match OpenAntryService::open_existing(data_dir.clone()) {
        Ok(s) => Ok(s),
        Err(_) => {
            eprintln!("note: data dir not initialized — running init");
            let (s, r) = OpenAntryService::init(data_dir)
                .map_err(|e| anyhow::anyhow!(e.to_string()))?;
            if let Some(key) = r {
