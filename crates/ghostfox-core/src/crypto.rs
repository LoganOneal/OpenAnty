...
impl MasterKey {
    ...
    pub fn load_or_create(data_dir: &Path) -> std::io::Result<(Self, Option<String>)> {
        ...
        if let Ok(env) = std::env::var("GHOSTFOX_MASTER_KEY") {
        ...
        let recovery = format!("GF-RECOVERY-{}", hex::encode(key));
        let recovery_path = data_dir.join("recovery.key.ONCE.txt");
        std::fs::write(
            &recovery_path,
            format!(
                "GhostFox recovery key — SAVE THIS OFFLINE, then delete this file.\n\n{recovery}\n"
            ),
        )?;
