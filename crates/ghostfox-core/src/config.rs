...
impl Default for Config {
    fn default() -> Self {
        Self {
            bind: std::env::var("GHOSTFOX_BIND").unwrap_or_else(|_| "127.0.0.1:3847".into()),
            sessions_cap: std::env::var("GHOSTFOX_MAX_SESSIONS")
                .ok()
                .and_then(|s| s.parse().ok())
                .unwrap_or(5),
            cdp_port_start: 9222,
            cdp_port_end: 9321,
            browser_path: std::env::var("GHOSTFOX_BROWSER_PATH").ok().filter(|s| !s.is_empty()),
            max_session_ttl_seconds: 86400,
            allow_lan: std::env::var("GHOSTFOX_ALLOW_LAN").ok().as_deref() == Some("1"),
            experimental_js_stealth: std::env::var("GHOSTFOX_EXPERIMENTAL_JS_STEALTH")
                .ok()
                .as_deref()
                == Some("1"),
        }
...
impl Config {
    ...
    pub fn save(&self, data_dir: &Path) -> std::io::Result<()> {
        ...
        let path = data_dir.join("config.toml");
        let text = format!(
            r#"# GhostFox daemon configuration
bind = "{}"
sessions_cap = {}
cdp_port_start = {}
cdp_port_end = {}
