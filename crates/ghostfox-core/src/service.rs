//! High-level GhostFox service: profiles, sessions, cookies, proxy.

use chrono::{Duration, Utc};
use ghostfox_fp::{fingerprint_hash, generate_with_overrides, validate};
use ghostfox_proto::*;
...
impl GhostfoxService {
    pub fn init(data_dir: PathBuf) -> Result<(Self, Option<String>), GhostfoxError> {
        crate::paths::ensure_dir(&data_dir).map_err(|e| {
            GhostfoxError::app(ErrorCode::Internal, e.to_string())
        })?;
    ...
    pub fn open_existing(data_dir: PathBuf) -> Result<Self, GhostfoxError> {
        if !data_dir.join("ghostfox.db").exists() && !data_dir.join("api.token").exists() {
            return Err(GhostfoxError::app(
                ErrorCode::NotInitialized,
                "GhostFox not initialized — run `ghostfox init` first",
            )
            .with_hint("ghostfox init"));
        }
    ...
    fn browser_info(&self) -> Result<BrowserInfo, GhostfoxError> {
        ...
        let info = browser::resolve_browser(self.config.browser_path.as_deref()).map_err(|e| {
            GhostfoxError::app(ErrorCode::BinaryMissing, e)
                .with_hint("Install Chrome or set browser_path in config / GHOSTFOX_BROWSER_PATH")
        })?;
    ...
    pub fn create_profile(&self, req: CreateProfileRequest) -> Result<Profile, GhostfoxError> {
        ...
        .map_err(|e| GhostfoxError::app(ErrorCode::FingerprintInconsistent, e))?;
        ...
        self.store
            .insert_profile(&row)
            .map_err(|e| GhostfoxError::app(ErrorCode::Internal, e))?;
        self.store.audit("profile.create", "profile", &id, "{}");
        Ok(self.to_profile(&row, false))
    }

    pub fn list_profiles(&self, limit: u32) -> Result<Vec<Profile>, GhostfoxError> {
        let rows = self
            .store
    ...
    pub fn get_profile(&self, id: &str, include_secrets: bool) -> Result<Profile, GhostfoxError> {
        let row = self
            .store
            .get_profile(id)
            .map_err(|e| GhostfoxError::app(ErrorCode::Internal, e))?
            .ok_or_else(|| GhostfoxError::app(ErrorCode::ProfileNotFound, "profile not found"))?;
        Ok(self.to_profile(&row, include_secrets))
    }

    pub fn delete_profile(&self, id: &str) -> Result<(), GhostfoxError> {
    ...
    ) -> Result<(Profile, ProxyStatus, bool), GhostfoxError> {
        let mut row = self
            .store
        ...
        if req.check {
            ...
            if req.align_geo {
                ...
                if let Some(country) = check.country.as_deref().or(status.country.as_deref()) {
                    let tz = ghostfox_fp::timezone_for_country(country);
                    let browser = self.browser_info().unwrap_or(BrowserInfo {
                        path: PathBuf::from("chrome"),
    ...
    pub fn import_cookies(
        ...
        cookies: Vec<Cookie>,
        merge: bool,
    ) -> Result<(u32, u32, bool), GhostfoxError> {
        let mut row = self
            .store
            .get_profile(profile_id)
            .map_err(|e| GhostfoxError::app(ErrorCode::Internal, e))?
    ...
    pub fn export_cookies(&self, profile_id: &str) -> Result<Vec<Cookie>, GhostfoxError> {
        let row = self
            .store
            .get_profile(profile_id)
            .map_err(|e| GhostfoxError::app(ErrorCode::Internal, e))?
    ...
    fn alloc_port(&self) -> Result<u16, GhostfoxError> {
        for _ in 0..40 {
        ...
        Err(GhostfoxError::app(
            ErrorCode::PortConflict,
            "no free CDP ports in configured range",
        ))
    }

    pub async fn launch_session(&self, req: LaunchSessionRequest) -> Result<Session, GhostfoxError> {
        ...
        if let Some(existing) = &profile.lock_session_id {
            if !req.force {
                ...
                profile = self
                    ...
                    .map_err(|e| GhostfoxError::app(ErrorCode::Internal, e))?
                    .ok_or_else(|| {
                        GhostfoxError::app(ErrorCode::ProfileNotFound, "profile not found")
                    })?;
            }
        }
        ...
        let cdp_ws_url = wait_for_cdp(port, 30).await.map_err(|e| {
            ...
            GhostfoxError::app(ErrorCode::Internal, e)
        })?;
    ...
    pub async fn stop_session(&self, session_id: &str) -> Result<(), GhostfoxError> {
        let mut ses = self
            .store
            .get_session(session_id)
            .map_err(|e| GhostfoxError::app(ErrorCode::Internal, e))?
            .ok_or_else(|| GhostfoxError::app(ErrorCode::SessionNotFound, "session not found"))?;
    ...
    pub fn get_session(&self, id: &str) -> Result<Session, GhostfoxError> {
        let ses = self
            .store
            .get_session(id)
            .map_err(|e| GhostfoxError::app(ErrorCode::Internal, e))?
    ...
    pub fn list_sessions(&self) -> Result<Vec<Session>, GhostfoxError> {
        let rows = self
            .store
            .list_sessions(false)
            .map_err(|e| GhostfoxError::app(ErrorCode::Internal, e))?;
    ...
    pub fn heartbeat(&self, session_id: &str) -> Result<Session, GhostfoxError> {
        let mut ses = self
            .store
            .get_session(session_id)
            .map_err(|e| GhostfoxError::app(ErrorCode::Internal, e))?
    ...
    fn session_to_api(&self, ses: &SessionRow) -> Result<Session, GhostfoxError> {
        let profile = self
            .store
            .get_profile(&ses.profile_id)
            .map_err(|e| GhostfoxError::app(ErrorCode::Internal, e))?
            .ok_or_else(|| GhostfoxError::app(ErrorCode::ProfileNotFound, "profile missing"))?;
        let report = validate(&profile.fingerprint);
