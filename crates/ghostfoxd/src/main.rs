...
#[derive(Parser, Debug)]
#[command(name = "ghostfoxd", version, about = "GhostFox local antidetect daemon")]
struct Cli {
...
async fn main() -> Result<()> {
    ...
    let service = Arc::new(
        GhostfoxService::open_existing(data_dir.clone()).or_else(|_| {
            tracing::info!("initializing GhostFox data dir at {}", data_dir.display());
            GhostfoxService::init(data_dir.clone()).map(|(s, recovery)| {
...
async fn serve(service: Arc<GhostfoxService>) -> Result<()> {
    ...
    tracing::info!("GhostFox API listening on http://{addr}");
...
fn write_daemon_meta(service: &GhostfoxService) -> Result<()> {
    ...
    std::fs::write(
        service.data_dir.join("ghostfoxd.pid"),
