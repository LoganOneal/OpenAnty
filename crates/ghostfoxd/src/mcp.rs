//! Minimal MCP stdio server (JSON-RPC 2.0) for GhostFox tools.
...
pub async fn run(service: Arc<GhostfoxService>) -> anyhow::Result<()> {
...
async fn handle_tool_call(service: Arc<GhostfoxService>, params: Value) -> Result<Value, String> {
    let name = params
        .get("name")
        .and_then(|n| n.as_str())
