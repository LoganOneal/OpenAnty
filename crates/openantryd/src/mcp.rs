//! Minimal MCP stdio server (JSON-RPC 2.0) for OpenAntry tools.
...
pub async fn run(service: Arc<OpenAntryService>) -> anyhow::Result<()> {
...
async fn handle_tool_call(service: Arc<OpenAntryService>, params: Value) -> Result<Value, String> {
    let name = params
        .get("name")
        .and_then(|n| n.as_str())
