...
#[derive(Clone)]
pub struct AppState {
    pub service: Arc<OpenAntryService>,
}

pub fn router(service: Arc<OpenAntryService>) -> Router {
...
fn auth(headers: &HeaderMap, service: &OpenAntryService) -> Result<(), ApiError> {
    if std::env::var("OPENANTRY_INSECURE_NO_TOKEN").ok().as_deref() == Some("1") {
        return Ok(());
    }
    ...
    let provided = headers
        ...
        .or_else(|| {
            headers
                .get("x-openantry-token")
                .and_then(|v| v.to_str().ok())
        });
    match provided {
        Some(t) if t == expected => Ok(()),
        _ => Err(ApiError::from_err(
            OpenAntryError::app(ErrorCode::Unauthorized, "missing or invalid API token"),
            OpenAntryService::request_id(),
        )),
    }
}
...
impl ApiError {
    fn from_err(err: OpenAntryError, request_id: String) -> Self {
        let body = err.body();
...
async fn list_profiles(
    ...
    host_ok(&headers)?;
    auth(&headers, &state.service)?;
    let rid = OpenAntryService::request_id();
    let items = state
        .service
